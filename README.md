...
轻量级 Telegram 会话验证码面板。挂载已有的 Telegram `.session` 文件后，服务会监听来自 `777000` 的消息，提取 5/6 位验证码，并通过一个带 Basic Auth 的 Web 面板展示。

Lightweight Telegram session OTP dashboard. Mount existing Telegram `.session` files, listen for messages from `777000`, extract 5/6-digit login codes, and display them in a Basic Auth protected web panel.
...
| 变量 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `AUTH_USER` | 是 | 无 | Web 面板 Basic Auth 用户名 |
| `AUTH_PASS` | 是 | 无 | Web 面板 Basic Auth 密码 |
...
- 当前界面鉴权是 Basic Auth，强烈建议放在 HTTPS 或反向代理后面使用
...
| --- | --- | --- | --- |
| `AUTH_USER` | Yes | none | Basic Auth username for the dashboard |
| `AUTH_PASS` | Yes | none | Basic Auth password for the dashboard |
...
- the UI uses Basic Auth, so HTTPS or a reverse proxy is strongly recommended
